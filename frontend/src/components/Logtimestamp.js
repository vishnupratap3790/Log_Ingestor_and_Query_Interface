...
const Logtimestamp = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbytimestamp?timestamp=${timestamp}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter Timestamp');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Retrieve logs related to a specific Timestamp</Title>
