...
//   const [alertMessage, setAlertMessage] = useState('');
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbylevel?level=${level}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter Input');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Perform Query Based on Level</Title>
