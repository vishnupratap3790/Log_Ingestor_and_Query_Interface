...
const Logdaterange = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbydaterange?startDate=${startDate}&endDate=${endDate}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Both Start and End Date is required');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>search within specific Date Ranges</Title>
