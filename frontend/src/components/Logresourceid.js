...
const Logresourceid = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbyresourceid?resourceId=${resourceId}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter Resource Id');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Retrieve logs related to a specific Resource ID</Title>
