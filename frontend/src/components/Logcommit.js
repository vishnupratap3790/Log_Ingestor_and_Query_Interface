...
const Logcommit = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbycommit?commit=${commit}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter Commit');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Search logs using Commit</Title>
