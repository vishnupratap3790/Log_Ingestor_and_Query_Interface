...
const Logspanid = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbyspanId?spanId=${spanId}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter Span Id');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Retrieve logs related to a specific Span ID</Title>
