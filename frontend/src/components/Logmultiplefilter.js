...
const Logmultiplefilter = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbymultiplefilter?level=${level}&message=${message}&resourceId=${resourceId}&traceId=${traceId}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter All Parameter');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Search by combining multiple filters</Title>
...
export default Logmultiplefilter ;
