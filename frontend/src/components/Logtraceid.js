...
const Logtraceid = () => {
  ...
  const handlePerformQuery = async () => {
    try {
      ...
      const response = await fetch(`/logs/searchbytraceId?traceId=${traceId}`,{
        method: 'GET'
      });
      const data = await response.json();
      if(response.status===400) swal('Please Enter Trace ID');
  ...
  return (
    <div>
      <Title style={{textAlign:"center"}}>Retrieve logs related to a specific Trace ID</Title>
